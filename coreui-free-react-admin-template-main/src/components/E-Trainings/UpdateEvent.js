import React, { useEffect, useRef, useState } from 'react'
import axios from 'axios'
import { storage } from '../../config/firebaseConfig'
import { ref, uploadBytes, getDownloadURL, deleteObject } from 'firebase/storage'

const UpdateEvent = ({ handleUpdateEventClose, getAllEvents, eventId }) => {
  const [fileURL, setFileURL] = useState(null)
  const [definitionData, setDefinitionData] = useState({})
  const [oldImage, setOldImage] = useState('')

  useEffect(() => {
    if (typeof definitionData.event_image === 'string') {
      setOldImage(definitionData.event_image)
    }
  }, [definitionData])

  const get_One_Event = async () => {
    try {
      const res = await axios.get(
        // `http://localhost:5000/api/get-One-Event/${eventId}`,
        `https://ma-training-consulting-company-site-backend.vercel.app/api/get-One-Event/${eventId}`,
      )

      if (res.data) {
        setDefinitionData(res.data)
      }
    } catch (error) {
      console.error(error)
    }
  }

  useEffect(() => {
    get_One_Event()
  }, [])

  const handleSubmitDefinitionData = async (e) => {
    e.preventDefault()

    try {
      let event_image_url = oldImage

      if (oldImage && fileURL) {
        const rerFile = ref(storage, oldImage)

        try {
          await deleteObject(rerFile)
          console.log('Old file deleted successfully.')
        } catch (error) {
          console.error('Failed to delete old file:', error)
        }
      }

      if (fileURL) {
        // Upload files to Firebase Storage
        const uploadFile = async (file, filePath) => {
          const storageRef = ref(storage, filePath)
          await uploadBytes(storageRef, file)
          return getDownloadURL(storageRef)
        }

        const randomNumber = Math.floor(Math.random() * 1000)

        event_image_url = definitionData.event_image
          ? await uploadFile(
              definitionData.event_image,
              `event/images/${definitionData.event_image.name}${randomNumber}`,
            )
          : null
      }

      const dataToSubmit = {
        ...definitionData,
        event_image: event_image_url,
      }

      const res = await axios.patch(
        // `http://localhost:5000/api/update-Event-Data/${eventId} `,
        `https://ma-training-consulting-company-site-backend.vercel.app/api/update-Event-Data/${eventId}`,
        dataToSubmit,
        {
          headers: {
            'Content-Type': 'application/json',
          },
        },
      )

      if (res.data) {
        getAllEvents()
        handleUpdateEventClose()
      }
    } catch (error) {
      console.log(error)
    }
  }

  const handleChangeDefinitionData = (e) => {
    const { name, value } = e.target
    setDefinitionData((prevState) => ({
      ...prevState,
      [name]: value,
    }))
  }

  const headerfileRef = useRef(null)

  const handleClickHeaderFile = () => {
    headerfileRef.current.click()
  }

  const handleChangeHeaderFile = (e) => {
    if (fileURL) {
      URL.revokeObjectURL(fileURL)
    }
    const file = e.target.files[0]
    if (file) {
      const newFileURL = URL.createObjectURL(file)
      setDefinitionData((prevState) => ({
        ...prevState,
        event_image: file,
      }))
      setFileURL(newFileURL)
    }
  }

  return (
    <div className="Definition">
      <div className="container">
        <div className="row tm-content-row tm-mt-big">
          <div className="col-12 tm-col-big">
            <div className="tm-block h-100 my-5 ">
              <h2 className="tm-block-title">événement</h2>
              <div className="header">
                <div className="row mt-4 tm-edit-product-row w-100">
                  <div className="col-xl-7 col-lg-7 col-md-12">
                    <form onSubmit={handleSubmitDefinitionData} className="tm-edit-product-form">
                      <div className="input-group mb-3">
                        <label
                          htmlFor="event_title"
                          className="col-xl-4 col-lg-4 col-md-4 col-sm-5 col-form-label"
                        >
                          titre
                        </label>
                        <input
                          name="event_title"
                          onChange={handleChangeDefinitionData}
                          type="text"
                          value={definitionData?.event_title}
                          className="form-control validate col-xl-9 col-lg-8 col-md-7 col-sm-7"
                        />
                      </div>

                      <div className="input-group mb-3">
                        <label
                          htmlFor="event_description"
                          className="col-xl-4 col-lg-4 col-md-4 col-sm-5 mb-2"
                        >
                          description
                        </label>
                        <textarea
                          className="form-control validate col-xl-9 col-lg-8 col-md-8 col-sm-7"
                          name="event_description"
                          onChange={handleChangeDefinitionData}
                          value={definitionData?.event_description}
                        ></textarea>
                      </div>

                      <div className="input-group mb-3">
                        <label
                          htmlFor="event_date"
                          className="col-xl-4 col-lg-4 col-md-4 col-sm-5 col-form-label"
                        >
                          Date
                        </label>
                        <input
                          name="event_date"
                          onChange={handleChangeDefinitionData}
                          type="date"
                          value={definitionData?.event_date}
                          className="form-control validate col-xl-9 col-lg-8 col-md-7 col-sm-7"
                        />
                      </div>

                      <div className="input-group mb-3 d-flex justify-content-center">
                        <div className="ml-auto col-xl-8 col-lg-8 col-md-8 col-sm-7 pl-0 text-center">
                          <button type="submit" className="btn btn-primary">
                            Ajouter
                          </button>
                        </div>
                      </div>
                    </form>
                  </div>
                  <div className="col-xl-4 col-lg-4 col-md-12 mx-auto mb-4 text-center ">
                    <div className="tm-product-img-dummy mx-auto">
                      {fileURL || definitionData?.event_image ? (
                        <span
                          style={{
                            display: 'flex',
                            flexDirection: 'column',
                            border: '.5px solid',
                            width: '200px',
                            height: '175px',
                            justifyContent: 'center',
                            margin: 'auto',
                          }}
                        >
                          <img
                            style={{
                              width: '100%',
                              height: '100%',
                            }}
                            id="bg-video"
                            src={fileURL ? fileURL : definitionData?.event_image}
                            type="video/mp4"
                          />
                        </span>
                      ) : (
                        <span
                          style={{
                            display: 'flex',
                            flexDirection: 'column',
                            border: '.5px solid',
                            width: '200px',
                            height: '175px',
                            justifyContent: 'center',
                            margin: 'auto',
                          }}
                        >
                          <i className="fas fa-5x fa-cloud-upload-alt"></i>
                        </span>
                      )}
                    </div>
                    <div className="custom-file mt-3 mb-3">
                      <input
                        ref={headerfileRef}
                        type="file"
                        accept="image/*"
                        onChange={handleChangeHeaderFile}
                        style={{ display: 'none' }}
                      />
                      <input
                        type="button"
                        className="btn btn-primary d-block mx-auto"
                        value="Upload ..."
                        onClick={handleClickHeaderFile}
                      />
                    </div>
                  </div>
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>
    </div>
  )
}

export default UpdateEvent
