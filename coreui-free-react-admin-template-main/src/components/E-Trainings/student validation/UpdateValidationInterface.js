...
const UpdateValidationInterface = ({
  ...
  useEffect(() => {
    setDefinitionData(validationInterface)
  }, [])
    ...
    try {
      ...
      if (fileURL) {
        ...
        inderface_image_url = await uploadFile(
          ...
          `validationInterface/images/${definitionData.inderface_image.name}`,
        )
      }
      ...
      const res = await axios.patch(
        // `http://localhost:5000/api/update-Validation-Interface/${definitionData?._id} `,
        `https://ma-training-consulting-company-site-backend.vercel.app/api/update-Validation-Interface/${definitionData?._id} `,
        dataToSubmit,
        {
          headers: {
        ...
        <div className="row tm-content-row tm-mt-big">
          <div className="col-12 tm-col-big">
            <div className="tm-block h-100 my-5 ">
              <h2 className="tm-block-title">Validation Interface</h2>
...
export default UpdateValidationInterface
