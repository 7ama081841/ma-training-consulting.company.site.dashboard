...
const UpdatePackDeFormation = ({ packDeFormation, setcheck }) => {
  ...
  const getDate = () => {
    setAvantageData(packDeFormation)
    if (packDeFormation.date) {
      const formattedDate = packDeFormation.date.split('T')[0]
  ...
  const handleSubmitAvantageData = async (e) => {
    ...
    try {
      ...
      const dataToSubmit = {
        ...avantageData,
        image: !fileURL ? avantageData.image : file_url,
      }

      const res = await Axios.patch(
        // `http://localhost:5000/api/update-PackDeFormation/${packDeFormation?._id} `,
        `https://ma-training-consulting-company-site-backend.vercel.app/api/update-PackDeFormation/${packDeFormation?._id} `,
        dataToSubmit,
        {
          headers: {
      ...
      if (res.data) {
        ...
        setAvantageData((prevState) => ({
          ...prevState,
          ...updatedPack,
          date: prevState.date.split('T')[0] || updatedPack.date.split('T')[0],
        }))
      }
    } catch (error) {
  ...
  const handleDeleteAvantageData = async (e) => {
    e.preventDefault()

    try {
      const res = await Axios.delete(
        // `http://localhost:5000/api/delete-PackDeFormation/${packDeFormation?._id}`,
        `https://ma-training-consulting-company-site-backend.vercel.app/api/delete-PackDeFormation/${packDeFormation?._id} `,
      )
              ...
              <div className="header">
                <div className="row mt-4 tm-edit-product-row w-100">
                    ...
                    <form onSubmit={handleSubmitAvantageData} className="tm-edit-product-form">
                      ...
                      <div className="input-group mb-3 d-flex justify-content-center ">
                        <div className="ml-auto col-xl-8 col-lg-8 col-md-8 col-sm-7 pl-0 text-center">
                          <CButton
                            className=" mx-3 "
                            onClick={handleDeleteAvantageData}
                            color="danger"
                            variant="outline"
                          >
                            supprimer
                  ...
                  <div className="col-xl-4 col-lg-4 col-md-12 mx-auto mb-4 text-center">
                    ...
                    ></div>
                    {fileURL || avantageData.image ? (
                      <img
                        style={{
                          width: '100%',
                        }}
                        id="bg-video"
                        src={fileURL ? fileURL : avantageData.image}
                      />
                    ) : (
                      <i
                        style={{
...
export default UpdatePackDeFormation
